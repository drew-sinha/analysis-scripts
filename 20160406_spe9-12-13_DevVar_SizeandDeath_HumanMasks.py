...
if __name__=='__main__':
    ...
    pxsize_conversion=1#1.304/1000    # mm; i.e. 1 px size is ~1.304 um
    ...
    for expt_dir in [subdir for subdir in sorted(os.listdir(top_dir)) if os.path.isdir(top_dir+os.path.sep+subdir)]:
        for worm_dir in sorted(os.listdir(top_dir+os.path.sep+expt_dir)):
            ...
            all_worm_sizes = all_worm_sizes*pxsize_conversion**2
            all_worm_sizechgs = all_worm_sizechgs*pxsize_conversion**2
    ...
    if do_withincorrelation:
        ...
        for stg in range(4):
            ...
            ax_h[stg].set_title('L{} - Mean: {:.0f}, Std: {:.0f}'.format(stg+1,np.mean(all_worm_sizes[:,stg+1]),np.std(all_worm_sizes[:,stg+1])))
        ...
        ax_h.set_ylabel('Mask size (px)')
        ax_h.set_xlim([0,4])
        ax_h.set_xticks(np.linspace(0,4,5))
        ax_h.xaxis.set_ticklabels(['Hatch','End_L1','E_L2','E_L3','E_L4'])
        
        # Plot mean centered size over time per individual
        fig_h = plt.figure()
        ax_h = fig_h.gca()
        [ax_h.plot(worm_sizes) for worm_sizes in (all_worm_sizes-np.mean(all_worm_sizes, axis=0))]
        ax_h.set_title('Size over time (mean-centered)')
        ax_h.set_ylabel('Size relative to mean (px)')
        ax_h.set_xlim([0,4])
        ax_h.set_xticks(np.linspace(0,4,5))
        ax_h.xaxis.set_ticklabels(['Hatch','End_L1','E_L2','E_L3','E_L4'])    
        ...
        # Scatter sizes against each other
        fig_h, ax_h = plt.subplots(4,4)
        fig_h.suptitle('Size per dev. stage')
        for stg_1 in range(3):
            for stg_2 in range(stg_1+1,4):
                ax_h[stg_1,stg_2].scatter(all_worm_sizes[:,stg_1+1],all_worm_sizes[:,stg_2+1])  # skip first element
        ...
        #######################
        # Plot distribution of size changes over all individuals
        plt.close('all')
        plt.gcf().clf()
        fig_h,ax_h = plt.subplots(4,1)
        fig_h.suptitle('Size changes distribution')
        for stg in range(4):
            ax_h[stg].hist(all_worm_sizechgs[:,stg])
            ax_h[stg].set_title('L{} - Mean: {}, Std: {}'.format(stg+1,np.mean(all_worm_sizechgs[:,stg]),np.std(all_worm_sizechgs[:,stg])))
            #ax_h[stg].set_title('L{}'.format(stg+1))
            if stg == 4:
                ax_h[stg].set_xlabel('Size change (px)')
                ax_h[stg].set_ylabel('Frequency')
        
        # Plot size changes over time per individual
        fig_h = plt.figure()
        ax_h = fig_h.gca()
        [ax_h.plot(worm_sizechgs) for worm_sizechgs in all_worm_sizechgs]
        ax_h.set_title('Size change over time per individual')
        ax_h.set_ylabel('Size change in stage (px)')
        ax_h.set_xlim([0,3])
        ax_h.set_xticks(np.linspace(0,3,4))
        ax_h.xaxis.set_ticklabels(['L1','L2','L3','L4'])
        
        # Plot mean centered size change over time per individual
        fig_h = plt.figure()
        ax_h = fig_h.gca()
        [ax_h.plot(worm_sizechgs) for worm_sizechgs in (all_worm_sizechgs-np.mean(all_worm_sizechgs, axis=0))]
        ax_h.set_title('Size changes per stage (mean-centered)')
        ax_h.set_ylabel('Size change relative to mean (px)')
        ax_h.set_xlim([0,3])
        ax_h.set_xticks(np.linspace(0,3,4))
        ax_h.xaxis.set_ticklabels(['L1','L2','L3','L4'])
        ...
        fig_h = plt.figure()
        ax_h = fig_h.gca()
        [ax_h.plot(worm_sizechgs) for worm_sizechgs in (all_worm_sizechgs-np.mean(all_worm_sizechgs, axis=0))/np.std(all_worm_sizechgs,axis=0)]
        ax_h.set_title('Size changes per stage (z-scored)')
        ax_h.set_ylabel('Size change (Z-score)')
        ax_h.set_xlim([0,3])
        ax_h.set_xticks(np.linspace(0,3,4))
        ax_h.xaxis.set_ticklabels(['L1','L2','L3','L4'])
        
        # Scatter size changes against each other
        fig_h, ax_h = plt.subplots(4,4)
        fig_h.suptitle('Size change per stage')
        for stg_1 in range(3):
            for stg_2 in range(stg_1+1,4):
                ax_h[stg_1,stg_2].scatter(all_worm_sizechgs[:,stg_1],all_worm_sizechgs[:,stg_2])
                ax_h[stg_1,stg_2].set_title('L{} vs. L{}'.format(stg_1+1,stg_2+1))
        
        # Spearman correlations on size changes
        print('Correlation on size changes (Spearman)')
        (corr_vals, corr_p_vals) = scipy.stats.spearmanr(all_worm_sizechgs)
        print(corr_vals)
        print(corr_p_vals)
        ...
        fig_h = plt.figure()
        ax_h = fig_h.gca()
        [ax_h.plot(worm_devtime) for worm_devtime in (all_worm_devtime-np.mean(all_worm_devtime, axis=0))]
        ax_h.set_title('Dev. stage duration per individual (mean-centered)')
        ax_h.set_ylabel('Dev. stage duration to mean (px)')
        ax_h.set_xlim([0,3])
        ax_h.set_xticks(np.linspace(0,3,4))
        ax_h.xaxis.set_ticklabels(['L1','L2','L3','L4'])
    ...
    if do_betweencorrelation:
        ...
        print(stats)
        print(p_vals)
        lin_stats = np.zeros(stats.shape)
        lin_pvals = np.zeros(p_vals.shape)
        for size_idx, worm_sizes in enumerate(all_worm_sizes[:,1:].T):
            for time_idx, worm_times in enumerate(all_worm_devtime.T):
                lin_stats[size_idx,time_idx], lin_pvals[size_idx,time_idx] = scipy.stats.pearsonr(worm_sizes, worm_times)
        print('Linear correlation - size(row) and time (col)')
        print(lin_stats)
        print(lin_pvals)
    ...
    if do_totalcorrelation:
        ...
        for lag in range(3):
            ...
            ax_h[lag].set_xlabel('Size[stg]')
            ax_h[lag].set_ylabel('Size[stg+lag]')
        ...
        for lag in range(3):
            ...
            ax_h[lag].set_xlabel('Size change [stg]')
            ax_h[lag].set_ylabel('Size change [stg+lag]')
        ...
        for lag in range(3):
            ...
            ax_h[lag].set_title('Stages {} away \n Spearman r={:.2f}, p={:.3f}'.format(lag+1,corr_val,p_val))
            ax_h[lag].set_xlabel('Growth rate [stg]')
            ax_h[lag].set_ylabel('Growth rate [stg+lag]')
        ...
        for lag in range(3):
            ...
            ax_h[lag].set_title('Stages {} away \n Spearman r={:.2f}, p={:.3f}'.format(lag+1,corr_val,p_val))
            ax_h[lag].set_xlabel('Stage duration [stg]')
            ax_h[lag].set_ylabel('Stage duration [stg+lag]')
    ...
    for stg in range(4):
        for event_num, (health_event,health_param) in enumerate(zip(worm_intra_events.T,health_params)):
            ...
            ax_h[stg,event_num].set_xlabel('Size (px)')
    ...
    for stg in range(4):
        for event_num, (health_event,health_param) in enumerate(zip(worm_intra_events.T,health_params)):
            ...
            ax_h[stg,event_num].set_xlabel('Rate (px)')
            ax_h[stg,event_num].set_ylabel(health_param)
    ...
    for stg in range(4):
        for event_num, (health_event,health_param) in enumerate(zip(worm_intra_events.T,health_params)):
            ...
            ax_h[stg,event_num].set_xlabel('Size change (px)')
            ax_h[stg,event_num].set_ylabel(health_param)
    ...
    for stg in range(4):
        for event_num, (health_event,health_param) in enumerate(zip(worm_intra_events.T,health_params)):
            ...
            ax_h[stg,event_num].set_xlabel('Larval stage duration (hr)')
            ax_h[stg,event_num].set_ylabel(health_param)
