...
if __name__=='__main__':
    ...
    for my_ax in ax_h:
        ...
        my_ax.xaxis.set_ticks_position('none')
        my_ax.yaxis.set_ticks_position('none')
    
    # Plot ending size against lifespan
    fig_h, ax_h = plt.subplots(4,1)
    fig_h.suptitle('Size at end of stage')
    for stg in range(4):
        fit_stats = scipy.stats.spearmanr(all_worm_sizes[:,stg+1],lifespans)
        ax_h[stg].set_title('L{} - \rho = {:.3f} p={:.3f}'.format(stg+1,fit_stats[0],fit_stats[1]))
        ax_h[stg].scatter(all_worm_sizes[:,stg+1],lifespans)
        #ax_h[stg].set_xlabel('Size (mm^2)')
        #ax_h[stg].set_ylabel('Lifespan (hr.)')
        ax_h_lims = ax_h[stg].get_xlim()
        ax_h[stg].set_xlim([max(ax_h_lims[0],0),ax_h_lims[1]])
        ax_h[stg].spines['top'].set_visible(False)
        ax_h[stg].spines['right'].set_visible(False)
        ax_h[stg].xaxis.set_ticks_position('none')
        ax_h[stg].yaxis.set_ticks_position('none')
    
    # Plot distribution of stage durations over all individuals
    fig_h,ax_h = plt.subplots(5,1)
    fig_h.suptitle('Larval stage duration')
    [devtime_min,devtime_max] = [all_worm_devtime.min(), all_worm_devtime.max()]
    for stg in range(5):
        if stg < 4:
            #ax_h[stg].hist(all_worm_devtime[:,stg])
            
            #ax_h[stg].hist(all_worm_devtime[:,stg],bins=np.arange(np.floor(all_worm_devtime[:,stg].min()),np.ceil(all_worm_devtime[:,stg].max()),0.5)) # GOOD
            ax_h[stg].hist(all_worm_devtime[:,stg],bins=np.arange(np.floor(devtime_min), np.ceil(devtime_max),0.5))
            ...
            mat_time = np.sum(all_worm_devtime,axis=1)
            ax_h[stg].hist(mat_time,bins=np.arange(np.floor(mat_time.min()),np.ceil(mat_time.max()),0.5))
            ax_h[stg].set_title('Time to adulthood - Mean: {:.2f}, Std: {:,.2f}'.format(np.mean(mat_time),np.std(mat_time)))
        ax_h[stg].spines['top'].set_visible(False)
        ax_h[stg].spines['right'].set_visible(False)
        ax_h[stg].xaxis.set_ticks_position('none')
        ax_h[stg].yaxis.set_ticks_position('none')
    ...
    for stg in range(5):
        if stg is not 4:
            fit_stats = scipy.stats.spearmanr(all_worm_devtime[:,stg],lifespans)
            ax_h[stg].set_title('L{} - \rho={:.3f} p={:.3f}'.format(stg+1,fit_stats[0],fit_stats[1]))
            ...
            mat_time = np.sum(all_worm_devtime,axis=1)
            ax_h[stg].set_title('Time to adulthood')
            ax_h[stg].scatter(mat_time,lifespans)
        ax_h[stg].spines['top'].set_visible(False)
        ax_h[stg].spines['right'].set_visible(False)
        ax_h[stg].xaxis.set_ticks_position('none')
        ax_h[stg].yaxis.set_ticks_position('none')
    ...
    ax_h.xaxis.set_ticks_position('none')
    ax_h.yaxis.set_ticks_position('none')
    
    #Size vs. size at later time
    #fig_h, ax_h = plt.subplots(1,3)
    #fig_h.suptitle('Autocorrelations (z-scored) in size over time')
    #for lag in range(3):
        ...
        #corr_val, p_val = scipy.stats.spearmanr(
            #np.concatenate([sizes_zscored[:,stg+1] for stg in range(4-lag-1)]),
            #np.concatenate([sizes_zscored[:,stg+lag+1+1] for stg in range(4-lag-1)]))
        
        #ax_h[lag].set_title('Stages {} away; rho = {:.3f}, p = {:.3f}'.format(lag+1,corr_val,p_val))
        #ax_h[lag].set_xlabel('Size[stg] (z-score)')
        #ax_h[lag].set_ylabel('Size[stg+lag] (z-score)')
        #ax_h[lag].spines['top'].set_visible(False)
        #ax_h[lag].spines['right'].set_visible(False)
        #ax_h[lag].xaxis.set_ticks_position('none')
        #ax_h[lag].yaxis.set_ticks_position('none')
    
    #Plot L1 vs. L4 size
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.set_title('Size - L1 vs. L4 size; rho = {:.3f}, p = {:.3f}'.format(*scipy.stats.spearmanr(sizes_zscored[:,1],sizes_zscored[:,-1])))
    ax_h.scatter(sizes_zscored[:,1],sizes_zscored[:,-1])
    ax_h.spines['top'].set_visible(False)
    ax_h.spines['right'].set_visible(False)
    ax_h.xaxis.set_ticks_position('none')
    ax_h.yaxis.set_ticks_position('none')
    
    # Plot stage durations as adjusted z_scores
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ...
    # Plot L1 vs. L4 stage duration
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.set_title('Timing - L1 vs. L4 duration; rho = {:.3f}, p = {:.3f}'.format(*scipy.stats.spearmanr(devtime_zscored[:,0],devtime_zscored[:,-1])))
    ax_h.scatter(devtime_zscored[:,0],devtime_zscored[:,-1])
    ax_h.spines['top'].set_visible(False)
    ax_h.spines['right'].set_visible(False)
    ax_h.xaxis.set_ticks_position('none')
    ax_h.yaxis.set_ticks_position('none')
    ...
    #fig_h, ax_h = plt.subplots(1,3)
    #fig_h.suptitle('Autocorrelations (z-scored) in larval stage duration over time')
    #for lag in range(3):
    ...
    #fig_h = plt.figure()
    #ax_h = fig_h.gca()
    #ax_h.scatter(sizes_zscored[:,1:-1].flatten(), sizechgs_zscored[:,1:].flatten())
    #(reg_slope,reg_int,reg_r,reg_p,reg_se) = scipy.stats.linregress(sizes_zscored[:,1:-1].flatten(), sizechgs_zscored[:,1:].flatten())
    #ax_h.set_title('Size changes[stg+1] vs. size[stg]\n sizechg[stg+1] = {:1.3f}*size[stg]+{:1.3f}'.format(reg_slope,reg_int))
    
    # Size and average growth rate
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.scatter(sizes_zscored[:,1:-1].flatten(), rates_zscored[:,1:].flatten())
    ax_h.set_title('Growth rate[stg+1] vs. sizes[stg];rho={:.3f}, p={:.3f}'.format(*scipy.stats.spearmanr(sizes_zscored[:,1:-1].flatten(), rates_zscored[:,1:].flatten())))
    ax_h.spines['top'].set_visible(False)
    ax_h.spines['right'].set_visible(False)
    ax_h.xaxis.set_ticks_position('none')
    ax_h.yaxis.set_ticks_position('none')
    ...
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.scatter(rates_zscored[:,0:-1].flatten(), rates_zscored[:,1:].flatten())
    ax_h.set_title('Growth rate[stg+1] vs. growth rate[stg]; rho={:.3f},p={:.3f}'.format(*scipy.stats.spearmanr(rates_zscored[:,0:-1].flatten(), rates_zscored[:,1:].flatten())))
    ax_h.spines['top'].set_visible(False)
    ax_h.spines['right'].set_visible(False)
    ax_h.xaxis.set_ticks_position('none')
    ax_h.yaxis.set_ticks_position('none')
    ...
    if do_totalcorrelation:
        ...
        (reg_slope,reg_int,reg_r,reg_p,reg_se) = scipy.stats.linregress(sizes_zscored[:,1:-1].flatten(), sizechgs_zscored[:,1:].flatten())
        ax_h[0].set_title('Timing[stg+1] vs. size[stg]\n timing[stg+1] = {:1.3f}*size[stg]+{:1.3f}\nPearson r={:.3f}, p ={:.3f}'.format(reg_slope,reg_int,reg_r,reg_p))
        #ax_h[0].set_title('Timing[stg+1] vs. size[stg]')
    ...
    # Cohort analysis
    cohort_size = len(goodworms)/4
    cohort_assignments = np.zeros((len(goodworms),4),dtype='int')
    indiv_fluct = np.zeros((len(goodworms),4))
    for stg in range(4):
        cohort_cutoffs = np.concatenate(([0],np.percentile(all_worm_sizes[:,stg+1],[25,50,75,100])))
        cohort_assignments[:,stg] = np.digitize(all_worm_sizes[:,stg+1], cohort_cutoffs,right=True).astype('int')-1 # Start number groups at 0
        cohort_means = np.array([np.mean(all_worm_sizes[cohort_assignments[:,stg]==group_num,stg+1]) for group_num in range(4)])
        print(cohort_means)
        indiv_fluct[:,stg] = all_worm_sizes[:,stg+1]-cohort_means[cohort_assignments[:,stg]]
    print(cohort_assignments)
    print(indiv_fluct)
    
    life_cutoffs = np.concatenate(([0],np.percentile(lifespans,[25,50,75,100])))
    cohort_life_assignments = np.digitize(lifespans,life_cutoffs,right=True)-1
    
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.scatter(np.sum(np.abs(indiv_fluct),axis=1),lifespans)
    
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.scatter(indiv_fluct[:,-1], lifespans)
    
    fig_h = plt.figure()
    ax_h = fig_h.gca()
    ax_h.scatter(np.abs(indiv_fluct[:,-1]), lifespans)
    
    # Plot size color-coded for eventual quantiles of lifepsan and adult size
    colors = [[1,0,0],
        [0,1,0],
        [0,0,1],
        [0,0,0]]
    ...
    fig_h,ax_h = plt.subplots(1,2)
    for worm_size,size_num,life_num,worm_lifespan in zip(all_worm_sizes, cohort_assignments, cohort_life_assignments, lifespans):
        ax_h[0].plot(np.arange(len(worm_size)),worm_size,color=colors[life_num])
        ax_h[1].plot(np.arange(len(worm_size)),worm_size,color=colors[size_num[-1]])
    
    
    # 1. How consistent is the membership of any one individual to a given cohort
    
    # Does flucutations between cohorts predict lifespan?
    
    # Does fluctuation within a cohort predict lifespan?
