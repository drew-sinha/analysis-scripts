...
def read_annotations(experiment_root,subdir='annotations'):
    ...
    experiment_root = pathlib.Path(experiment_root)
    positions = collections.OrderedDict()
    for annotation_file in sorted((experiment_root / subdir).glob('*.pickle')):
...
def write_annotations(experiment_root, positions, subdir='annotations'):
    ...
    """
    annotation_dir = pathlib.Path(experiment_root) / subdir
